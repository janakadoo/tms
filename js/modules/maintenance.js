...
export const MaintenanceModule = {
    ...
    _bindEvents() {
        ...
        document.querySelectorAll('[data-view]').forEach(btn => {
            btn.onclick = () => {
                const r = DB.Maintenance.getById(btn.dataset.view);
                if (!r) return;
                const html = `
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem;">
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">DATE</strong><br>${r.date ? Utils.formatDate(r.date) : '—'}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">VEHICLE</strong><br>${Utils.esc(r.reg_no || '—')}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">TYPE</strong><br>${Utils.esc(r.type)}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">WORKSHOP</strong><br>${Utils.esc(r.workshop || r.garage || '—')}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">COST</strong><br><span style="color:var(--danger);font-weight:600;">${Utils.currency(r.cost)}</span></div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">ODOMETER</strong><br>${r.odometer ? (+r.odometer).toLocaleString()+' km' : '—'}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">STATUS</strong><br>${Utils.maintStatusBadge(r.status)}</div>
                        <div><strong style="color:var(--text-muted);font-size:0.75rem;">NEXT DUE</strong><br>${r.next_due_date ? Utils.formatDate(r.next_due_date) : '—'} <br> ${r.next_due_km ? r.next_due_km + ' km' : ''}</div>
                    </div>
                    <div>
                        ...
                        <div style="background:var(--bg-input);padding:0.75rem;border-radius:var(--r-md);margin-top:0.5rem;min-height:60px;">
                            ${Utils.esc(r.description || 'No remarks provided.')}
                        </div>
                    </div>
                `;
                Utils.viewDialog('Maintenance Details', html);
            };
        });
